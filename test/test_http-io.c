...
void test__http_begin_request__writes_the_request_line_with_empty_query(void)
{
    int fd = open_tmp_file();
    TEST_ASSERT_GREATER_OR_EQUAL(0, fd);
...
void test__http_begin_request__sends_host_header(void)
{
    int fd = open_tmp_file();
    TEST_ASSERT_GREATER_OR_EQUAL(0, fd);
    ...
    http_begin_request(&request);
    TEST_ASSERT_STRING_CONTAINS_SUBSTRING("Host: www.example.com\r\n", get_file_content(fd));
...
void test__http_begin_request__has_GET_as_default_method(void)
{
    int fd = open_tmp_file();
    TEST_ASSERT_GREATER_OR_EQUAL(0, fd);
...
void test__http_begin_request__writes_the_POST_request_line(void)
{
    int fd = open_tmp_file();
    TEST_ASSERT_GREATER_OR_EQUAL(0, fd);
    ...
    http_begin_request(&request);
    TEST_ASSERT_EQUAL_STRING_PREFIX("POST / HTTP/1.1\r\n", get_file_content(fd));
