...
void http_sha1_update(struct http_sha1_ctx * context, const uint8_t *data, uint32_t len)
{
    ...
    if ((j + len) > 63)
    {
        ...
        http_sha1_transform(context->state, context->buffer);
        for (; i + 63 < len; i += 64)
        {
            http_sha1_transform(context->state, &data[i]);
        }
        j = 0;
    }
    else
...
#endif
    c = 0200;
    http_sha1_update(context, &c, 1);
    while ((context->count[0] & 504) != 448)
    {
        c = 0000;
        http_sha1_update(context, &c, 1);
    }
    http_sha1_update(context, finalcount, 8); /* Should cause a SHA1Transform() */
    for (i = 0; i < 20; i++)
    {
        digest[i] = (uint8_t)
...
void http_sha1(uint8_t *hash_out, const char *str, int len)
{
    ...
    for (ii=0; ii<len; ii+=1)
        http_sha1_update(&ctx, (const uint8_t*)str + ii, 1);
    http_sha1_final((uint8_t *)hash_out, &ctx);
    hash_out[20] = '\0';
}
